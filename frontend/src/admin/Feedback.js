...
const Feedback = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        // Fetch Room Reports
        const roomResponse = await axios.get("http://localhost:3001/room-reports");
        console.log("Fetched Room Reports:", roomResponse.data);
        setRoomReports(Array.isArray(roomResponse.data) ? roomResponse.data : []);

        // Fetch User Reports
        const userResponse = await axios.get("http://localhost:3001/reports");
  ...
  const handleResolveChange = async (reportId, currentResolved) => {
    ...
    try {
      await axios.patch(`http://localhost:3001/reports/update/${reportId}`, {
  ...
  const handleApprovalChange = async (reportId, currentApproval) => {
    ...
    try {
      await axios.patch(`http://localhost:3001/room-reports/update/${reportId}`, {
  ...
  const removeReport = async (reportId, isRoomReport = true) => {
    ...
    try {
      if (isRoomReport) {
        await axios.delete(`http://localhost:3001/room-reports/delete/${reportId}`);
        setRoomReports((prevReports) =>
          prevReports.filter((report) => report.room_report_id !== reportId)
        );
      } else {
        await axios.delete(`http://localhost:3001/reports/delete/${reportId}`);
        setUserReports((prevReports) =>
  ...
  const removeAllReports = async (isRoomReport = true) => {
    try {
      if (isRoomReport) {
        await axios.delete("http://localhost:3001/room-reports/delete-all");
        setRoomReports([]);
      } else {
        await axios.delete("http://localhost:3001/reports/delete-all");
        setUserReports([]);
      }
    } catch (error) {
                        ...
                        <td>
                          {report.image && report.image !== "No image uploaded" ? (
                            <img
                              ...
                              onClick={() =>
                                setSelectedImage(`http://localhost:3001${report.image}`)
                              }
                            />
                          ) : (
                            "No Image"
